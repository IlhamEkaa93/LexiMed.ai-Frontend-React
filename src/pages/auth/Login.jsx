...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Animasi transisi sebelum pindah halaman (300ms)
        setTimeout(() => {
  ...
  // Fungsi Dummy Account (Otomatis Submit)
  const setDummyAccountAndLogin = (role) => {
    setError("");
    let tempUsername = "";
    ...
    switch (role) {
      case "dokter":
        tempUsername = "dokter1";
        break;
      case "perawat":
        tempUsername = "perawat1";
        break;
      case "radiologi":
        tempUsername = "radiologi1";
        break;
      case "manajemen":
        tempUsername = "manajemen1";
        break;
      case "asisten":
        tempUsername = "asisten1";
        break;
      case "admin":
        tempUsername = "admin1";
        break;
    ...
    // Set state dan langsung trigger login
    setUsername(tempUsername);
    setPassword(tempPassword);
    
    // Perlu sedikit delay agar state React ter-update sebelum disubmit
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans selection:bg-blue-500 selection:text-white">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 md:p-24 bg-white relative shadow-[-20px_0_40px_-15px_rgba(0,0,0,0.1)] z-20">
        {/* Tombol Kembali */}
        <button 
          onClick={() => navigate("/")}
        ...
        >
          ...
          <div className="mb-10 text-center lg:text-left">
            <div className="lg:hidden w-16 h-16 bg-gradient-to-br from-blue-600 to-emerald-500 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/30 mx-auto mb-6">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <button
              ...
              type="submit"
              disabled={isLoading}
              className="w-full bg-slate-900 text-white font-bold rounded-xl p-4 mt-2 hover:bg-blue-600 transition-all flex justify-center items-center gap-2 shadow-xl shadow-slate-900/10 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed group"
            >
              {isLoading ? (
          ...
          <div className="mt-12 pt-8 border-t border-slate-100">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <button onClick={() => setDummyAccountAndLogin("dokter")} type="button" className="flex flex-col items-center justify-center p-3 rounded-xl bg-slate-50 hover:bg-blue-50 border border-slate-100 hover:border-blue-200 text-slate-600 hover:text-blue-600 transition-all group">
              ...
              <button onClick={() => setDummyAccountAndLogin("perawat")} type="button" className="flex flex-col items-center justify-center p-3 rounded-xl bg-slate-50 hover:bg-emerald-50 border border-slate-100 hover:border-emerald-200 text-slate-600 hover:text-emerald-600 transition-all group">
                <Activity size={20} className="mb-2 group-hover:scale-110 transition-transform" />
                <span className="text-xs font-bold">Perawat</span>
              </button>
              <button onClick={() => setDummyAccountAndLogin("radiologi")} type="button" className="flex flex-col items-center justify-center p-3 rounded-xl bg-slate-50 hover:bg-purple-50 border border-slate-100 hover:border-purple-200 text-slate-600 hover:text-purple-600 transition-all group">
              ...
              <button onClick={() => setDummyAccountAndLogin("manajemen")} type="button" className="flex flex-col items-center justify-center p-3 rounded-xl bg-slate-50 hover:bg-amber-50 border border-slate-100 hover:border-amber-200 text-slate-600 hover:text-amber-600 transition-all group">
              ...
              <button onClick={() => setDummyAccountAndLogin("asisten")} type="button" className="flex flex-col items-center justify-center p-3 rounded-xl bg-slate-50 hover:bg-indigo-50 border border-slate-100 hover:border-indigo-200 text-slate-600 hover:text-indigo-600 transition-all group">
              ...
              <button onClick={() => setDummyAccountAndLogin("admin")} type="button" className="flex flex-col items-center justify-center p-3 rounded-xl bg-slate-900 text-white hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/20 group">
